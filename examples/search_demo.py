...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Index demo docs (if needed) and run a hybrid search query.",
    )
    ...
    parser.add_argument(
        "--recreate",
        action="store_true",
        help="Drop + recreate collection before indexing demo docs",
    )
    ...
    dense_dim = len(hybrid_search._to_list(next(iter(dense_model.embed(["dimension probe"])))))
    if args.recreate:
        hybrid_search.recreate_collection(client, args.collection, dense_dim)
    else:
        hybrid_search.ensure_collection(client, args.collection, dense_dim)

    info = client.get_collection(args.collection)
    if args.recreate or (info.points_count or 0) == 0:
